...
<body class="bg-gray-50 dark:bg-gray-900 min-h-screen">
    <div class="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
                <div class="flex justify-between items-start mb-4">
                    ...
                    <div class="flex gap-2 items-center">
                        @php
                            $totalApostado = $corridaSelecionada->apostas->sum(function($aposta) {
                                return ($aposta->valor ?? 0) + ($aposta->lo ?? 0);
                            });
                        @endphp
                        <div class="px-4 py-2 bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-700 rounded-lg">
                            ...
                            <span class="ml-2 text-lg font-bold text-blue-600 dark:text-blue-400">
                                R$ {{ number_format($totalApostado, 2, ',', '.') }}
                            </span>
                        </div>
                        <button
            ...
            <!-- Seções Copiar para WhatsApp e Prêmios por Animal -->
            @if($corridaSelecionada->apostas->count() > 0)
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <!-- Seção Copiar para WhatsApp -->
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
                    <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-4">Lista de valores a pagar</h3>
                @php
                    // Agrupar apostas por apostador e calcular total de cada um
                    $apostadoresComTotal = $corridaSelecionada->apostas->groupBy('apostador_id')->map(function($apostas) {
                        $apostador = $apostas->first()->apostador;
                        $total = $apostas->sum(function($aposta) {
                            return ($aposta->valor ?? 0) + ($aposta->lo ?? 0);
                        });
                        return [
                            'nome' => $apostador->nome,
                    ...
                    foreach($apostadoresComTotal as $item) {
                        ...
                        $tamanhoValor = mb_strlen($textoValor);
                        $tracosNecessarios = max(1, $larguraTotal - $tamanhoNome - $tamanhoValor);
                        $tracos = str_repeat('-', $tracosNecessarios);
                        $textoWhatsApp .= $nome . $tracos . $textoValor . "\n\n";
                    }
                @endphp
                <div class="space-y-4">
                    <!-- Accordion para texto -->
                    <div>
                        <button
                            onclick="toggleAccordion('accordion-whatsapp')"
                            class="w-full flex items-center justify-between px-4 py-3 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 rounded-lg transition-colors">
                            <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Ver texto formatado</span>
                            <svg id="icon-accordion-whatsapp" class="w-5 h-5 text-gray-500 dark:text-gray-400 transform transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                            </svg>
                        </button>
                        <div id="accordion-whatsapp" class="hidden mt-2">
                            <div class="bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                                <pre id="texto-whatsapp" class="text-sm text-gray-800 dark:text-gray-200 whitespace-pre-wrap font-mono">{{ $textoWhatsApp }}</pre>
                            </div>
                        </div>
                    </div>
                    <!-- Botão de copiar sempre visível -->
                    <button
                        onclick="copiarParaWhatsApp(event)"
                ...
                <!-- Seção Prêmios por Animal -->
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
                    <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-4">Prêmios por Animal</h3>
                @php
                    $todasApostas = $corridaSelecionada->apostas;
                    $taxa = $corridaSelecionada->taxa / 100;
                    
                    // Agrupar apostas por animal
                    $apostasPorAnimal = $todasApostas->groupBy('animal')->sortKeys();
                    
                    $premiosPorAnimal = [];
                    
                    foreach($apostasPorAnimal as $animal => $apostasAnimal) {
                        // Agrupar apostas por rodada
                        $apostasPorRodada = $apostasAnimal->groupBy('rodada');
                        
                        $premiosApostadores = [];
                        
                        foreach($apostasPorRodada as $rodada => $apostasRodadaAnimal) {
                            // Buscar todas as apostas da rodada (todos os animais)
                            $todasApostasRodada = $todasApostas->where('rodada', $rodada);
                            
                            // Calcular total apostado na rodada
                            $totalRodada = $todasApostasRodada->sum(function($a) {
                                return ($a->valor ?? 0) + ($a->lo ?? 0);
                            });
                            
                            // Prêmio líquido da rodada (após taxa)
                            $premioLiquidoRodada = $totalRodada * (1 - $taxa);
                            
                            // Total apostado no animal nesta rodada
                            $totalApostadoAnimalRodada = $apostasRodadaAnimal->sum(function($a) {
                                return ($a->valor ?? 0) + ($a->lo ?? 0);
                            });
                            
                            if ($totalApostadoAnimalRodada > 0) {
                                // Para cada apostador que apostou no animal nesta rodada
                                $apostasPorApostador = $apostasRodadaAnimal->groupBy('apostador_id');
                                
                                foreach($apostasPorApostador as $apostadorId => $apostasApostador) {
                                    $primeiraAposta = $apostasApostador->first();
                                    if (!$primeiraAposta || !$primeiraAposta->apostador) {
                                        continue;
                                    }
                                    
                                    $apostador = $primeiraAposta->apostador;
                                    
                                    // Total apostado pelo apostador no animal nesta rodada
                                    $totalApostadoApostador = $apostasApostador->sum(function($a) {
                                        return ($a->valor ?? 0) + ($a->lo ?? 0);
                                    });
                                    
                                    // Proporção e prêmio
                                    $proporcao = $totalApostadoApostador / $totalApostadoAnimalRodada;
                                    $premio = $premioLiquidoRodada * $proporcao;
                                    
                                    // Acumular prêmio do apostador para este animal
                                    if (!isset($premiosApostadores[$apostadorId])) {
                                        $premiosApostadores[$apostadorId] = [
                                            'nome' => $apostador->nome ?? 'Desconhecido',
                                            'premio' => 0
                                        ];
                                    }
                                    $premiosApostadores[$apostadorId]['premio'] += $premio;
                                }
                            }
                        }
                        
                        // Ordenar por nome do apostador
                        usort($premiosApostadores, function($a, $b) {
                            return strcmp($a['nome'], $b['nome']);
                        });
                        
                        $premiosPorAnimal[$animal] = $premiosApostadores;
                    }
                    ...
                    foreach($premiosPorAnimal as $animal => $apostadores) {
                        ...
                        $textoCabecalhoPremio = "Prêmio";
                        $tamanhoCabecalhoApostador = mb_strlen($textoCabecalhoApostador);
                        $tamanhoCabecalhoPremio = mb_strlen($textoCabecalhoPremio);
                        
                        // Os valores R$ terminam na posição $larguraTotal
                        // Alinhar "Prêmio" para terminar na mesma posição (alinhamento à direita)
                        $posicaoFinalPremio = $larguraTotal;
                        $posicaoInicioPremio = $posicaoFinalPremio - $tamanhoCabecalhoPremio;
                        ...
                        foreach($apostadores as $dados) {
                            ...
                            $tamanhoNome = mb_strlen($nome);
                            $tamanhoValor = mb_strlen($textoValor);
                            // Calcular traços para alinhar o valor na mesma posição
                            $tracosNecessarios = max(1, $larguraTotal - $tamanhoNome - $tamanhoValor - 1); // -1 para o ">"
                            $tracos = str_repeat('-', $tracosNecessarios);
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <button
                            onclick="toggleAccordion('accordion-premios')"
                            class="w-full flex items-center justify-between px-4 py-3 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 rounded-lg transition-colors">
                            <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Ver texto formatado</span>
                            <svg id="icon-accordion-premios" class="w-5 h-5 text-gray-500 dark:text-gray-400 transform transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                            </svg>
                        </button>
                        <div id="accordion-premios" class="hidden mt-2">
                            <div class="bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                                <pre id="texto-premios" class="text-sm text-gray-800 dark:text-gray-200 whitespace-pre-wrap font-mono">{{ $textoPremios }}</pre>
                            </div>
                        </div>
                    </div>
                    <!-- Botão de copiar sempre visível -->
                    <button
                        onclick="copiarPremios(event)"
                        class="w-full px-6 py-3 bg-green-600 hover:bg-green-700 text-white rounded-lg font-medium transition-colors flex items-center justify-center gap-2">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"></path>
                        </svg>
                        Copiar Prêmios para WhatsApp
                    </button>
                </div>
                </div>
            </div>
    ...
    <script>
        ...
        // Função para copiar texto para WhatsApp
        function copiarParaWhatsApp(event) {
            const textoElement = document.getElementById('texto-whatsapp');
            ...
            navigator.clipboard.writeText(texto).then(function() {
                ...
                if (botao) {
                    ...
                    botao.innerHTML = '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg> Copiado!';
        ...
        // Função para copiar prêmios por animal para WhatsApp
        function copiarPremios(event) {
            const textoElement = document.getElementById('texto-premios');
            if (!textoElement) return;
            ...
            navigator.clipboard.writeText(texto).then(function() {
                ...
                if (botao) {
                    const textoOriginal = botao.innerHTML;
                    botao.innerHTML = '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg> Copiado!';
                    botao.classList.remove('bg-blue-600', 'hover:bg-blue-700');
                    botao.classList.add('bg-blue-500');
                    
                    setTimeout(function() {
                        botao.innerHTML = textoOriginal;
                        botao.classList.remove('bg-blue-500');
                        botao.classList.add('bg-blue-600', 'hover:bg-blue-700');
                    }, 2000);
                }
            }).catch(function(err) {
                console.error('Erro ao copiar:', err);
